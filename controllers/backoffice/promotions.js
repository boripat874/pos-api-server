...
exports.productslist = (req, res) => {
  ...
  const productsListLogic = new Promise(async (resolve, reject) => {
    try {
      ...
      const {shopid} = req.body; // ค่าเริ่มต้นคือไม่มีค่า
      ...
      .select("productid","productCgtyId", "productnameth","create_at")
      .where({"status": true, "shopid": shopid})
      .orderBy("create_at", "desc");
...
exports.promotionCreate = async (req, res) => {
  ...
  const promotionCreateLogic = new Promise(async (resolve, reject) => {
    ...
    // console.log("productidlist >> ",req.body.productidlist);
    const productidlist = req.body.productidlist;
    // console.log("productidlist",typeof productidlist);
    try {
      ...
      if(typeof productidlist !== "object") {

        return reject({ status: 402, message: "Invalid request productidlist is not object array" });
      ...
      await db("userinfo")
        ...
        .then(async (result) => {
          ...
          if(ugroupid !== "admin"){
            ...
            return reject({ status: 402, message: "Admin don't have permission to access" });
          }
        });
...
// detials promotion
exports.promotionDetail = async (req, res) => {
  ...
  const promotionDetailLogic = new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      const promotion = await db("promoinfo").where({ promoid });

      if (!promotion) {
        return reject({ status: 404, message: "Promotion not found" });
      }
      ...
      const promotionproductslist = await Promise.all(promotion.map(async (promotionproduct) => {
          
        const product = await pdpromoidlist(promoid);

        return {
          promoid: promotionproduct.promoid,
          promoname: promotionproduct.promoname,
          shopid: promotionproduct.shopid,
          typepromotions: promotionproduct.typepromotions,
          discount: promotionproduct.discount,
          datepromostart: promotionproduct.datepromostart,
          datepromoend: promotionproduct.datepromoend,
          promoremark: promotionproduct.promoremark,
          initial: promotionproduct.initial,
          productidlist: product,
        }

      }))

      resolve(promotionproductslist[0]);
  ...
  Promise.race([promotionDetailLogic, timeoutPromise])
    .then((data) => {
      return res.send(data); // ส่ง response และหยุดการทำงาน
    })
    .catch((error) => {
...
exports.promotionDelete = async (req, res) => {
  ...
  Promise.race([promotionDeleteLogic, timeoutPromise])
    .then(async(data) => {
      ...
      await eventlog(req,"ยกเลิกโปรโมชั่น") // บันทึก log
